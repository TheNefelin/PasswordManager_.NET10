...
- ✅ **Tests**: 44/44 en total (unit 21 sin BD + integración con BD local y `Auth_Register` actualizado). Casos borde incluidos: login con usuario inexistente (401), password inválida (401), `IsEnableRegister=0` (403), IV con password errónea (401), lockout (bloqueo a los 5 fallos, expiración 1h, ventana 10 min, reset, aislamiento por IP).
